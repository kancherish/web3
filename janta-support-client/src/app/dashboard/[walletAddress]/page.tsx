...
import { JANTASUPPORT_FACTORY } from "@/app/constants/contracts";
...
export default function DashboardPage() {
    ...
    // Get Campaigns
    const { data: myCampaigns, isPending: isPendingCampaigns ,refetch } = useReadContract({
        contract,
                ...
                {!isPendingCampaigns && (
                    myCampaigns && myCampaigns.length > 0 ? (
                        myCampaigns.map((campaign, index) => (
                            <MyCampaignCard
                                key={index}
                                contractAddress={String(campaign.campaignAddress)}
                            />
                        ))
                    ) : (
                        <p>No campaigns</p>
    ...
    const handleDeployContract = async () => {
        ...
        try {
            ...
            const contractAddress = await deployPublishedContract({
                ...
                chain: defineChain(11155111),
                account: account!,
                contractId: "JantaSupportMain",
                contractParams:{
                    _name: campaignName,
                    _description: campaignDescription,
                    _goal: BigInt(campaignGoal).toString(), // Convert to wei and then to string
                    _durationInDays: campaignDeadline // Convert to Unix timestamp and then to string
                },
                publisher: "0x3e8fB1d7Ff0c76f19E924791918070f1AebEf374",
                version: "1.0.0",
            });
